...
import { dashboardProjectAPImock } from "./api/dashboard/project-api";
import { employeeeManagementApi } from "./api/experiment/employeemanagement-api";
...
function MockAdapterProvider(props: MockAdapterProviderProps) {
  ...
  useEffect(() => {
    const setupAllMocks = () => {
      [authApiMocks, dashboardProjectAPImock, employeeeManagementApi].forEach(
        (mockSetup) => {
          mockSetup(mock);
        }
      );
